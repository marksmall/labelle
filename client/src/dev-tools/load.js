const loadDevTools = callback => {
  ...
  // we want it enabled by default everywhere but production and we also
  // want to support the dev tools in production (to make us more productive
  ...
  if (
    ...
    // use a dynamic import so the dev-tools code isn't bundled with the
    // regular app code so we don't worry about bundle size.
    import('./tools')
      .then(tools => tools.install())
      .finally(callback);
  } else {
